...
\talktext
The stabilizer checks detect every error acting on one physical qubit.
A phase error at one site changes the all-site flip check's outcome to $-1$.

Now put phase errors at addresses $000$ and $100$. Each anticommutes with the all-site flip, so together their two sign changes cancel. The pair has zero syndrome.

The logical word $f_{100,0}=u_1$ is one at $100$ and zero at $000$. The error pair therefore changes the relative phase of the encoded state labeled by $a=100$:
\[
f_{100,0}(000)\oplus f_{100,0}(100)=0\oplus1=1.
\]
\takeaway{No one-site error is undetected, but this two-site phase error is undetected and acts logically. The toy code therefore has quantum distance two.}
\end{frame}
...
\talktext
The paper studies CSS codes with three logical qubits and phase distance $d_Z\ge3$.
The class also obeys nine Campbell--Howard parity conditions on individual generator rows and their pair and triple intersections.
